...
def prune_min(t):
    ...
    if t.is_leaf():
        return
    t1 = min(t.branches, key=lambda b: b.label)
    t.branches = [t1]
    if t1.is_leaf():
        return
    else:
...
def delete(t, x):
    ...
    for b in t.branches:
        delete(b, x)
        if b.label == x:
            new_branches.extend(b.branches)  # If b is deleted, its branches are added to the parent
        else:
            new_branches.append(b)  # If b is not deleted, it is kept in the new branches list

...
def addition(args):
    total = calc_eval(args.first)
    rest = args.rest
    while rest is not nil:
        total += calc_eval(rest.first)
        rest = rest.rest
    return total
...
def floor_div(args):
    ...
    if args.rest is not nil:
        numerator = args.first
        denominators = args.rest
        while denominators is not nil:
            denominator = calc_eval(denominators.first)
            numerator = numerator // denominator
            denominators = denominators.rest
        return numerator
    else:
        return 1//numerator
...
def eval_and(expressions):
    ...
    if expressions is not nil:
        ...
        while val is not scheme_f and expressions.rest is not nil:
...
def eval_define(expressions):
    ...
    if rest is nil:
        value = None
    elif rest.rest is nil:
        value = calc_eval(rest.first)
    else:
        value = []
        while rest is not nil:
            value.append(calc_eval(rest.first))
            rest = rest.rest

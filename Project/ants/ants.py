...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not self.damage_is_doubled:
            self.damage = 2 * self.damage
            self.damage_is_doubled = True
        # END Problem 12
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # 调用父类方法减少生命值（触发父类逻辑，如打印 "reduced"）
        super().reduce_health(amount)
        
        # 如果本次伤害导致死亡，施加额外伤害
        if current_health - amount <= 0:
            if current_place is not None:  # 确保未被移除
                for bee in current_place.bees[:]:
...
class ContainerAnt(Ant):
    ...
    def remove_from(self, place):
        ...
        if place.ant is self:
            ...
            assert place.ant is not None, '{0} is not in {1}'.format(self, place)
        # Clear the container's reference to the place
        self.place = None
